# dsh-vision（fork：自定义端点版）

> 这是 [@oil-oil/dsh-vision](https://github.com/oil-oil/dsh-vision) 的 fork 定制版，随 DSH Desktop 分发。
> **与原版的区别**：
> - 移除内置云平台（ZenMux / 百炼 / TokenDance / OpenRouter），只保留 **自定义** 端点选项（任意 OpenAI 兼容 `/chat/completions` 视觉模型，如小米 MiMo、智谱 GLM-4V、通义千问 VL 等）
> - 视觉 API Key 通过凭据 `DSH_VISION_CUSTOM_API_KEY` 提供
> - 观察结果内联到消息中，超长会话上下文下不丢失
> - 只分析最新消息中的图片，历史图片不重复调用视觉模型

配置：DSH 设置 → 插件 → 插件配置 → 视觉识别，选择「自定义」，填写 API 地址、模型 ID 与 API Key（Key 也可直接写入 `~/.dsh/.credentials.yaml` 的 `DSH_VISION_CUSTOM_API_KEY`）。
